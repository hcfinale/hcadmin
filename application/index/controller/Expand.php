...
class Expand extends Base
{
    ...
    public function avatarUpload()//用户头像上传方法
    {
        ...
        $info = $file->move('./public/avatar');
                
        if ($info) {
            $path = $info->getSaveName();

            $user = new User;
            $user->save(['avatar'=>'/public/avatar/'.$path], ['uid'=>input('post.uid')]);

            return json(array('code'=>0,'url'=>'/public/avatar/'.$path,'msg'=>'上传成功！','file'=>$file->getinfo()['name']));
        } else {
            return json(array('code'=>1,'errmsg'=>'上传失败'));
        }
    }
