...
class Api extends Base
{
    ...
    public function ebookList($page=1, $type='ebook', $fid=0)
    {
        $ebook = new Ebook;
        $list = $ebook->EbookList($page, $type, $fid);
        return json(['code'=>'0','data'=>$list[0],'pages'=>$list[1]]);
    }
    ...
    public function commentList($tid, $page=1)
    {
        ...
        foreach ($list[0] as $key => $value) {
            // empty($list[0][$key]['content']);
            // $list[0][$key]['content'] = htmlspecialchars_decode($list[0][$key]['content']);
            // // return $list[0][$key]['content'];
